...
def handle_connect(user_id, table, connection_id):
    """
    ...
    status_code = 200
    try:
        table.put_item(Item={"connectionId": connection_id, "user_id": user_id})
...
def handle_message(table, connection_id, event_body, apig_management_client):
    ...
    connection_ids = []
    target_user = event_body.get("target_user")
    try:
...
def lambda_handler(event, context):
    ...
    """
    print(f"event : {event}")
    table_name = "fastapi-chatapp-activeusers"
    ...
    if route_key == "$connect":
        user_id = event.get("queryStringParameters", {"name": "guest"}).get("name")
        response["statusCode"] = handle_connect(user_id, table, connection_id)
    elif route_key == "$disconnect":
        response["statusCode"] = handle_disconnect(table, connection_id)
    elif route_key == "sendmessage":
        body = event.get("body")
        body = json.loads(body if body is not None else '{"msg": "Got no message"}')
        domain = event.get("requestContext", {}).get("domainName")
        stage = event.get("requestContext", {}).get("stage")
        if domain is None or stage is None:
            logger.warning(
                "Could not send message. Bad endpoint in request domain '%s', "
                "stage '%s'",
                domain,
                stage,
            )
